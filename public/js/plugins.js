$(document).ready(function() {
    ...
    if ($("table.sortable").length > 0) {
        let iDisplayLength = 5,dataIDL = $("table.sortable").attr('data-idl');
        if (dataIDL) {
            iDisplayLength = dataIDL;
        }
		console.log(`data-idl: ${dataIDL}, idl: ${iDisplayLength}`);
    ...
    if ($("#wizard_validate").length > 0) {
        ...
        $('#wizard_validate').stepy({
            back: function(index) {//if(!$("#wizard_validate").validationEngine('validate')) return false; //uncomment if u need to validate on back click                
            },
            next: function(index) {
